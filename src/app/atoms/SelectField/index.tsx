...
const SelectField: React.FC<SelectFieldProps> = ({ label, data, onSelect }) => {
  ...
  const [buttonMeasurements, setButtonMeasurements] = useState({ top: 0, left: 0, width: 0 });
  ...
  const openSelect = (): void => {
    SelectButton.current.measure((fx: number, fy: number, width: number, height: number, px: number, py: number) => {
  ...
  return (
    <View>
      <TouchableOpacity ref={SelectButton} style={[styles.button, {width: buttonMeasurements.width || '90%'}]} onPress={toggleSelect}>
      ...
      {visible && (
        <Modal visible={visible} transparent animationType="none">
          <TouchableOpacity style={styles.overlay} onPress={() => setVisible(false)}>
            <View style={[styles.select, { top: selectTop, left: buttonMeasurements.left, width: buttonMeasurements.width }]}>
              <FlatList
                data={data}
